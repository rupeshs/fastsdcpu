...
def try_enable_npu_turbo(device, core):
    ...
    if "windows" in platform.system().lower():
        if "NPU" in device and "3720" not in core.get_property(
            "NPU", "DEVICE_ARCHITECTURE"
        ):
            try:
        ...
        if os.path.isfile("/sys/module/intel_vpu/parameters/test_mode"):
            with open("/sys/module/intel_vpu/version", "r") as f:
                ...
                if tuple(map(int, version.split("."))) < tuple(
                    ...
                    print(
                        f"The driver intel_vpu-1.9.0 (or later) needs to be loaded for NPU Turbo (currently {version}). Skipping..."
                    )
                else:
                    with open(
                        "/sys/module/intel_vpu/parameters/test_mode", "r"
                    ) as tm_file:
                        ...
                        if test_mode == 512:
                            ...
                            print(
                                "The driver >=intel_vpu-1.9.0 was must be loaded with "
                                '"modprobe intel_vpu test_mode=512" to enable NPU_TURBO '
                                f"(currently test_mode={test_mode}). Skipping..."
                            )
        else:
            print(
                f'The driver >=intel_vpu-1.9.0 must be loaded with  "modprobe intel_vpu test_mode=512" to enable NPU_TURBO. Skipping...'
            )
    else:
...
class StableDiffusionEngineAdvanced(DiffusionPipeline):
    def __init__(
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
            futures = {
                "unet_time_proj": executor.submit(
                    self.core.compile_model,
                    os.path.join(model, "unet_time_proj.xml"),
                    device[0],
                ),
                ...
                "unet_neg": executor.submit(
                    self.load_model, model, "unet_int8", device[2]
                )
                if device[1] != device[2]
                else None,
                "vae_decoder": executor.submit(
                    self.load_model, model, "vae_decoder", device[3]
                ),
                "vae_encoder": executor.submit(
                    self.load_model, model, "vae_encoder", device[3]
                ),
            }
        ...
        self.unet_neg = (
            futures["unet_neg"].result() if futures["unet_neg"] else self.unet
        )
        ...
        self.time_proj_constants = np.load(
            os.path.join(model, "time_proj_constants.npy")
        )
        ...
        for i, t in enumerate(self.progress_bar(timesteps)):
            ...
            if self.unet_neg.input("latent_model_input").shape[1] != 4:
                ...
                try:
                    ...
                    latent_model_input_neg = latent_model_input_neg.transpose(
                        0, 2, 3, 1
                    )
            ...
            time_proj_dict = {
                "sine_t": np.float32(sine_t),
                "cosine_t": np.float32(cosine_t),
            }
            self.infer_request_time_proj.start_async(time_proj_dict)
            self.infer_request_time_proj.wait()
            time_proj = self.infer_request_time_proj.get_output_tensor(0).data.astype(
                np.float32
            )

            input_tens_neg_dict = {
                "time_proj": np.float32(time_proj),
                "latent_model_input": latent_model_input_neg,
    ...
    def prepare_latents(
        ...
        if image is None:
            ...
            if isinstance(scheduler, LMSDiscreteScheduler):
                ...
                return noise, {}
            elif isinstance(scheduler, EulerDiscreteScheduler) or isinstance(
...
class StableDiffusionEngine(DiffusionPipeline):
    def __init__(
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=8) as executor:
            ...
            vae_en_future = executor.submit(
                self.load_model, model, "vae_encoder", device[3]
            )

            if self.batch_size == 1:
                if "int8" not in model:
                    unet_future = executor.submit(
                        self.load_model, model, "unet_bs1", device[1]
                    )
                    unet_neg_future = (
                        executor.submit(self.load_model, model, "unet_bs1", device[2])
                        if device[1] != device[2]
                        else None
                    )
                else:
                    unet_future = executor.submit(
                        self.load_model, model, "unet_int8a16", device[1]
                    )
                    unet_neg_future = (
                        executor.submit(
                            self.load_model, model, "unet_int8a16", device[2]
                        )
                        if device[1] != device[2]
                        else None
                    )
            ...
            self._vae_d_output = self.vae_decoder.output(0)
            self._vae_e_output = (
                self.vae_encoder.output(0) if self.vae_encoder else None
            )

            self.unet_input_tensor_name = (
                "sample"
                if "sample" in self.unet.input(0).names
    ...
    def __call__(
        ...
        latents, meta = self.prepare_latents(
            init_image, latent_timestep, scheduler, model
        )
        ...
        for i, t in enumerate(self.progress_bar(timesteps)):
            ...
            if self.batch_size == 1:
                ...
                if self.unet.input(self.unet_input_tensor_name).shape[1] != 4:
                    try:
                        latent_model_input_pos = latent_model_input_pos.permute(
                            0, 2, 3, 1
                        )
                    except:
                        latent_model_input_pos = latent_model_input_pos.transpose(
                            0, 2, 3, 1
                        )

                if self.unet_neg.input(self.unet_input_tensor_name).shape[1] != 4:
                    try:
                        latent_model_input_neg = latent_model_input_neg.permute(
                            0, 2, 3, 1
                        )
                    except:
                        latent_model_input_neg = latent_model_input_neg.transpose(
                ...
                if "sample" in self.unet_input_tensor_name:
                    input_tens_neg_dict = {
                        "sample": latent_model_input_neg,
                        "encoder_hidden_states": np.expand_dims(
                            text_embeddings[0], axis=0
                        ),
                        "timestep": np.expand_dims(np.float32(t), axis=0),
                    }
                    input_tens_pos_dict = {
                        "sample": latent_model_input_pos,
                        "encoder_hidden_states": np.expand_dims(
                            text_embeddings[1], axis=0
                        ),
                        "timestep": np.expand_dims(np.float32(t), axis=0),
                    }
                else:
                    input_tens_neg_dict = {
                        ...
                        "t": np.expand_dims(np.float32(t), axis=0),
                    }
                    input_tens_pos_dict = {
                        "latent_model_input": latent_model_input_pos,
                        "encoder_hidden_states": np.expand_dims(
                            text_embeddings[1], axis=0
                        ),
                        "t": np.expand_dims(np.float32(t), axis=0),
...
def print_npu_turbo_art():
    ...
    if random_number == 1:
        ...
        print(
            " /::::::::\__\ /:/_/:/  /  /\  \  \:\__\          /:/__/      /\  \  \:\__\ /:/_/:/__/___ /:/_/:/\:|__| /:/__/ \:\__\ "
        )
        print(
            " \:\~~\~~\/__/ \:\/:/  /   \:\  \ /:/  /         /::\  \      \:\  \ /:/  / \:\/:::::/  / \:\/:/ /:/  / \:\  \ /:/  / "
        )
